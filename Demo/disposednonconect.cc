...
void timer_handler()
{
    //定时处理任务，调用tick
    timer_lst.tick();
    //一次alarm只会引起一次SIGALRM信号，所以重新定时，不断触发SIGALRM;
    alarm(TIMESLOT);
}
    ...
    while (!stop_server)
    {
        ...
        for (int i = 0; i < number; i++)
        {
            ...
            if (sockfd == listenfd)
            {
                ...
                users[connfd].address = clilent_address;
